...
def dance():
    ...
    print(f"dance _ id:{dance_id}, {multiprocessing. current_process()")
    # 获取当前进程对象的父编号
    dãnce_ parent _ id =os.getppid()
    print(f"dance _ process的父编号是: {dance _ parent _ id}")
    for i in range(3):
        print("跳舞任务执行中…")
        time.sleep(0.5)
# 唱歌任务
def sing():
    ...
    # 获取当前进程对象的父编号
    singparent_id = os.getppid()
    print(f"sing _ process的父编号是:{sing_parent_id}")
    for i in range(3):
        print("唱歌任务执行中…")
        time.sleep(0.5)
"""
